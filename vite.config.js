...
function d2sWatcherPlugin() {
  return {
    ...
    configureServer(server) {
      ...
      // Scan all backup subfolders for corrupted .d2s and .d2i files
      server.middlewares.use('/__backup_scan', async (_req, res) => {
        try {
          const backupsDir = path.join(SAVES_DIR, 'backups')
          if (!fs.existsSync(backupsDir)) {
            res.writeHead(200, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ backupFolders: 0, filesScanned: 0, corrupted: [], ok: 0 }))
            return
          }
          ...
          for (const folder of folders) {
            ...
            const files = fs.readdirSync(folderPath).filter(f => f.endsWith('.d2s') || f.endsWith('.d2i'))
            ...
            for (const file of files) {
              ...
              try {
                ...
                if (file.endsWith('.d2s')) {
                  await parseD2S(filePath)
                } else {
                  await parseD2I(filePath)
                }
                ok++
              } catch (err) {
                corrupted.push({ folder, file, type: file.endsWith('.d2s') ? 'd2s' : 'd2i', error: err.message })
              }
            }
          }

          res.writeHead(200, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({ backupFolders: folders.length, filesScanned, corrupted, ok }))
        } catch (err) {
          res.writeHead(500, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({ error: err.message }))
        }
      ...
      // Delete specific corrupted files from backups and remove now-empty folders
      server.middlewares.use('/__backup_delete_corrupted', async (req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            for (const { folder, file } of files) {
              try {
                const filePath = path.join(backupsDir, folder, file)
                if (fs.existsSync(filePath)) {
                  fs.unlinkSync(filePath)
                  deleted.push({ folder, file })
                }
              } catch (err) {
                errors.push({ folder, file, error: err.message })
              }
            }

            // Remove any backup subfolders that are now empty
            ...
            res.writeHead(200, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ success: true, deleted, removedFolders, errors }))
          } catch (err) {
            res.writeHead(500, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ success: false, error: err.message }))
          }
