...
export class ItemSearchService {
  ...
  async search(input = {}) {
    const filters = normalizeItemSearchFilters(input)
    const query = filters.q.toLowerCase(), cap = Math.min(Math.max(+input.limit || 10, 1), 50), errors = [], characters = [], sharedStash = []
    ...
    if (filters.scope === 'all' || filters.scope === 'character') await Promise.all(saves.map(async filename => { const file = path.join(this.savesDir, filename); try { const stat = fs.statSync(file), save = await this.parse(file, this.parseD2S), sets = [[save.items,'character'],[save.contained_items,'contained'],[save.merc_items,'mercenary'],[save.corpse_items,'corpse'],[save.iron_golem_item?[save.iron_golem_item]:[],'iron-golem']]; for (const [items,kind] of sets) for (const row of walk(items,{sourceKind:'character',filename,characterName:save.name,fileSize:stat.size,fileMtimeMs:stat.mtimeMs,location:kind})) { if(row.location==='character')row.location=location(row.item); const projection = projectVaultEntry({ itemData: row.item }); const match = itemMatchesFilters(row.item, projection, filters) && getItemQueryMatch(row.item,query); if(match)characters.push(this.result({...row,match})) } } catch(e) { errors.push({sourceKind:'character',filename,error:e.message}) } }))
    if ((filters.scope === 'all' || filters.scope === 'sharedStash') && filters.sharedFile) { const sharedFile = filters.sharedFile; if (path.basename(sharedFile)!==sharedFile || path.extname(sharedFile).toLowerCase()!=='.d2i') errors.push({sourceKind:'sharedStash',filename:sharedFile,error:'sharedFile must be a .d2i basename'}); else { const file=path.join(this.savesDir,sharedFile); try { const stat=fs.statSync(file), stash=await this.parse(file,this.parseD2I); for(const [pageIndex,page] of (stash.pages||[]).entries())for(const row of walk(page.items,{sourceKind:'sharedStash',filename:sharedFile,fileSize:stat.size,fileMtimeMs:stat.mtimeMs,location:'shared-stash',pageIndex})){const projection=projectVaultEntry({itemData:row.item});const match=itemMatchesFilters(row.item,projection,filters)&&getItemQueryMatch(row.item,query);if(match)sharedStash.push(this.result({...row,match}))} } catch(e){errors.push({sourceKind:'sharedStash',filename:sharedFile,error:e.message})} } }
    let infiniteTotal = 0
    const infiniteStash = []
    if (filters.scope === 'all' || filters.scope === 'infiniteStash') {
      let cursor = null
      do {
        const page = this.repository.list({ q: query, category: filters.category, slot: filters.slot, setName: filters.setName, quality: filters.quality, minLevel: filters.minLevel, maxLevel: filters.maxLevel, cursor, limit: 50, statuses: ['active', 'pending_deposit', 'pending_withdraw'] })
        for (const entry of page.items) {
          const match = getItemQueryMatch(entry.itemData, query)
          if (match) infiniteStash.push(this.result({ sourceKind: 'infiniteStash', filename: entry.sourceSave, location: 'vault', vaultId: entry.vaultId, item: entry.itemData, match }))
        }
        cursor = page.nextCursor
      } while (cursor)
      infiniteTotal = infiniteStash.length
    }
    const group = (rows, total = rows.length) => { const ranked=rows.sort((a,b)=>a.match.rank-b.match.rank||a.preview.displayName.localeCompare(b.preview.displayName)); return {results:ranked.slice(0,cap),total} }
    return {query, filters, groups:{characters:group(characters),sharedStash:group(sharedStash),infiniteStash:group(infiniteStash, infiniteTotal)},errors,nextCursor:null}
  }
}
