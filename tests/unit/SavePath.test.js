...
test('accepts contained save basenames and rejects traversal or wrong extensions', () => {
  ...
  assert.equal(safeSavePath(root, 'Chosen.d2i', ['.d2s', '.d2i']), path.join(root, 'Chosen.d2i'))
  assert.equal(safeSavePath(root, 'Paladin.d2s', ['.d2s', '.d2i']), path.join(root, 'Paladin.d2s'))
  ...
  assert.throws(() => safeSavePath(root, 'C:Chosen.d2i', '.d2i'), /basename/)
  assert.throws(() => safeSavePath(root, 'con.d2s', ['.d2s', '.d2i']), /basename/)
  assert.throws(() => safeSavePath(root, 'nul.d2i', ['.d2s', '.d2i']), /basename/)
})